...
with open('setting.json', 'r') as f2:
    date = json.load(f2)
digits_aes = date['digits_aes']  # AES密钥位数
digits_rsa = date['digits_rsa']  # RSA密钥位数
photo1 = None
photo2 = None

def record_log(log):
    with open(f"client_file//{user}_log.txt", 'a+') as file:
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        file.write(user + "@" + timestamp + ":\t" + log + '\n')
...
def exchange_key_request(conn: socket.socket):
    ...
    aes = AES_ECB(digits_aes)

    log4 = "AES key:" + bytes(aes.key).hex()
    record_log(log4)
...
def exchange_key_response(conn: socket.socket):
    ...
    if re == "Exchange Key Request":
        ...
        aes = AES_ECB(digits_aes, eval(aes_key))
        conn.send(b"Verification Success")
        print("response: verification success")


def creat_image(image_user):
    # image_path = 图片路径
    if image_user == 'dz':
        image_path='pcs/2.png'
    elif image_user == 'wy':
        image_path='pcs/3.png'
    else:
        image_path='pcs/1.jpg'
...
def send(listbox, conn: socket.socket):
    ...
    listbox.window_create(tkinter.END, window=creat_image(user))
...
def receive(listbox, conn: socket.socket):
    ...
    while True:
        try:
            ...
            listbox.insert(tkinter.END, "\n")
            listbox.insert(tkinter.END, " " * 48 + timestamp + '\n', "tag4")
            listbox.window_create(tkinter.END, window=creat_image(other_user))
...
loginButton = tkinter.Button(root0, text="文件传输", command=switch_file_transfer, bg="Yellow")
...
# 建立连接
my_rsa = RSA.RSA(digits_rsa)  # 生成自身RSA密钥对
...
log3 = "get certification: " + str(certification.hex())
record_log(log3)
...
listbox1.insert(tkinter.END, '\n')
listbox1.insert(tkinter.END, "\n--------群通知--------\n")
listbox1.insert(tkinter.END, "[更多]:https://github.com/leipuxitele/encode_transform.git")
sendButton = tkinter.Button(root1, text="\n发\n\n\n送", anchor="n", command=lambda: send(listbox, s),
                            font=("Helvetica", 18), bg="white")
sendButton.place(x=585, y=320, width=55, height=300)

# 加载图片
image1 = Image.open("pcs/7.png")
image1 = image1.resize((507, 50))
photo = ImageTk.PhotoImage(image1)
label = tkinter.Label(root1, image=photo)
label.grid(row=0, column=0)  # 或者使用label.grid()来控制位置
