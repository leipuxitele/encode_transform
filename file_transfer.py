...
def record_log(log: str, addr: tuple):
    ...
    with open('server_file\\log.txt', 'a') as file:
...
class file_transfer:
    ...
    def __init__(self, conn: socket.socket, addr: tuple, aes: AES_ECB):
        """
        conn: socket连接
    ...
    def client_put(self, file_path):
        ...
        data_hear = {'filename': file_name,
                     'filesize': file_size, 'filesha256': file_sha_256}
    ...
    def server_send(self, file_save_path):
        ...
        if os.path.exists(file_save_path + filename):
            ...
            data_header = {'filename': filename,
                           'filesize': filesize, 'filesha256': filesha256}
